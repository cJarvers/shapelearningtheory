...
# local imports
sys.path.append("..")
from shapelearningtheory.analysis.rsa import get_shape_RDMs, get_texture_RDMs, get_model_RDMs
from shapelearningtheory.analysis.plots import plot_rdm_grid
from shapelearningtheory.datasets import make_dataset
from shapelearningtheory.networks import make_convnet_small, TextureConvNet, SRectangleConvNet

# Load dataset
traindata = make_dataset("rectangles", "stripes", "small", "standard")
traindata.prepare_data()
testdata = make_dataset("rectangles", "stripes", "small", "random")
...
# Generate RDMs for shape and texture features
print("Getting shape RDMs")
shape_rdms = get_shape_RDMs(testdata.val)
print("Getting texture RDMs")
texture_rdms = get_texture_RDMs(testdata.val)

# Load constructed models and generate RDMs for them
texture_net = TextureConvNet()
shape_net = SRectangleConvNet()
print("Getting feature network RDMs")
texturenet_rdms = get_model_RDMs(texture_net, testdata.test_dataloader(), use_image=False)
texturenet_rdms.dissimilarity_measure = "Euclidean"
texturenet_rdms.rdm_descriptors["property"] = texturenet_rdms.rdm_descriptors["layer"]
texturenet_rdms.rdm_descriptors["feature_type"] = ["texturenet"] * len(texturenet_rdms.rdm_descriptors["layer"])
...
del texture_net, shape_net

# Combine RDMs into one object and plot them
feature_rdms_list = [shape_rdms, texture_rdms, texturenet_rdms, shapenet_rdms]
feature_rdms = rsatoolbox.rdm.concat(*feature_rdms_list)
#print("Plotting")
figpath = "figures/experiment_3b/" 
os.makedirs(figpath, exist_ok=True)
#fig = plot_rdm_grid(feature_rdms_list)
#fig.savefig(figpath + "feature_rdms.png", bbox_inches="tight")
#plt.clf()
del feature_rdms_list, texture_rdms, shape_rdms, texturenet_rdms, shapenet_rdms
...
perform_rsa(net, testdata, feature_rdms, "epoch 10")

trainer.fit_loop.max_epochs = 20
trainer.fit(net, traindata)
print("Performing RSA after 20 epochs")
perform_rsa(net, testdata, feature_rdms, "epoch 20")